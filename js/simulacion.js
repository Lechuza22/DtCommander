...
(function () {
  ...
  const TRAIL_MIN = 8;
  const SNAP_DIST = 20;
  const TRAIL_COLORS = { player: '#ffffff', rival: '#fca5a5', ball: '#facc15' };
  ...
  const TEMPLATES = [
    {
      ...
      steps: [
        ...
        { name: 'Pase a la defensora', note: 'Pase al costado para sacar la pelota de la presión.', dur: 1.5, ball: 'DefI', our: { MedI: [50, 255] }, riv: { 7: [95, 300], 8: [170, 285] } },
        {
        ...
        {
          name: 'Pase al medio', note: 'Pase interior a la mediocampista y a jugar hacia adelante.', dur: 1.5, ball: 'MedC',
    ...
    },
    {
      ...
      steps: [
        ...
        { name: 'Pase a la banda', note: 'La pelota sale rápido al costado derecho.', dur: 1.5, ball: 'MedD', our: { MedD: [235, 200], DelD: [190, 118] }, riv: { 6: [228, 168], 3: [190, 92] } },
        {
        ...
        { name: 'Centro y remate', note: 'Centro al área y definición de la delantera.', dur: 1.6, ball: 'DelD', our: { DelD: [172, 74], MedD: [240, 118] } },
        { name: 'Remate', note: 'Remate al arco.', dur: 1, ballAt: [152, 18] }
      ]
    },
    {
      ...
      steps: [
        ...
        { name: 'Pase largo al costado', note: 'Pase largo por encima de la presión hacia la mediocampista de la derecha.', dur: 2, ball: 'MedD', our: { MedD: [240, 240], DelD: [205, 185] }, riv: { 6: [238, 208] } },
    ...
    },
    {
      ...
      steps: [
        ...
        { name: 'Pase a la compañera', note: 'Descarga de primera para la delantera que llega en diagonal.', dur: 1.2, ball: 'DelD', our: { DelD: [195, 120], DelI: [85, 145] }, riv: { 3: [225, 95], 5: [160, 150] } },
        { name: 'Definición', note: 'Conduce hacia el área y define ante la arquera.', dur: 1.5, ball: 'DelD', our: { DelD: [175, 72] }, riv: { 3: [215, 80] } },
        { name: 'Remate', note: 'Remate al arco.', dur: 1, ballAt: [152, 16] }
      ]
    },
    {
      ...
      steps: [
        ...
        { name: 'Pase a la pared', note: 'Pase corto a la delantera y arranque de la mediocampista.', dur: 1.2, ball: 'DelD', our: { MedC: [160, 222] } },
        { name: 'Devolución al espacio', note: 'De primera, la delantera devuelve detrás de la defensa.', dur: 1.6, ball: 'MedC', our: { MedC: [188, 148], DelD: [208, 180] }, riv: { 5: [150, 205], 8: [192, 205], 3: [205, 110] } },
  ...
  // Se calculan, no se guardan. progress (0-1) las hace crecer durante la animación.
  function autoTrails(from, to, progress, alpha) {
    const before = new Map(resolvedPhase(from).filter(isMovable).map(i => [keyOf(i), i]));
    const out = [];
    resolvedPhase(to).filter(isMovable).forEach(now => {
      const prev = before.get(keyOf(now));
      if (!prev) return;
      ...
      const reach = dist * progress;
      const headLen = reach - (now.type === 'ball' ? 8 : TOKEN_R + 3);
      if (headLen < 6) return;
      const ux = dx / dist;
      const uy = dy / dist;
      out.push({
        id: 'auto:' + keyOf(now) + ':' + to, type: 'arrow', _auto: true,
        x1: prev.x, y1: prev.y, x2: round1(prev.x + ux * headLen), y2: round1(prev.y + uy * headLen),
        color: TRAIL_COLORS[now.type], dash: now.type === 'ball', _o: alpha
      });
    });
    return out;
  }
  ...
  function staticFrame(ph) {
    const items = resolvedPhase(ph);
    return autoOn && ph > 0 ? items.concat(autoTrails(ph - 1, ph, 1, 1)) : items;
  }
  ...
  function transitionFrame(ph, p) {
    ...
    const toMap = new Map(to.map(i => [keyOf(i), i]));
    const seen = new Set();
    const out = [];
    from.forEach(a => {
    ...
    to.forEach(b => { if (!seen.has(keyOf(b))) out.push(Object.assign({}, b, { _o: e })); });
    if (autoOn) {
  ...
  function syncBalls() {
    const tokens = new Map(phaseItems(curPhase).filter(isToken).map(i => [keyOf(i), i]));
    phaseItems(curPhase).forEach(b => {
  ...
  function renamePlayer(oldName, newName) {
    mutate(() => {
      board.items.forEach(i => {
        ...
        if (i.type === 'ball' && i.carrier === 'p:' + oldName) i.carrier = 'p:' + newName;
      });
    });
    refreshAll();
  }
  ...
  function assignRoster() {
    ...
    mutate(() => {
      assignments.forEach(([role, name]) => {
        board.items.forEach(i => {
          if (i.type === 'player' && i.name === role) i.name = name;
          if (i.type === 'ball' && i.carrier === 'p:' + role) i.carrier = 'p:' + name;
        });
      });
    });
    refreshAll();
  ...
  function addPhase() {
    ...
    mutate(() => {
      ...
      const copies = board.items.filter(i => i.ph === cur).map(i => Object.assign(clone(i), { ph: cur + 1 }));
  ...
  function updateSelBar() {
    ...
    if (item.type === 'ball') {
      ...
      $('simSelLabel').textContent = holder ? `Pelota, la lleva ${holder.type === 'rival' ? 'rival ' + holder.label : holder.name}` : 'Pelota (suelta)';
      rename.hidden = true;
      giveBtn.hidden = true;
      releaseBtn.hidden = !item.carrier;
    } else {
      $('simSelLabel').textContent = item.type === 'rival' ? `Rival ${item.label}` : `Jugadora ${item.name}`;
      giveBtn.hidden = false;
      releaseBtn.hidden = true;
      rename.hidden = item.type !== 'player';
      if (item.type === 'player') {
